...
@Directive({
  selector: 'input[appAgeNumber]'
})
export class AgeNumberDirective {
  private prevVal: any;
  ...
  @HostListener('input', ['$event']) onInputChange(event: any) {
    const initalValue = this.el.nativeElement.value;
    const c = initalValue.replace(/^((?!(0))[0-9]{0,2})$/g, '');
    this.el.nativeElement.value = initalValue.substr(0, 2);
    if (c.length > 1 && initalValue.length > 2) {
      if (this.prevVal) {
